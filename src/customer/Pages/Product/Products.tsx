import React, { useEffect, useRef, useState } from 'react'
import FilterSection from './FilterSection'
import ProductCard from './ProductCard'
import { Box, Divider, FormControl, IconButton, InputLabel, MenuItem, Pagination, Select, ThemeProvider, useMediaQuery, useTheme } from '@mui/material'
import FilterAltIcon from '@mui/icons-material/FilterAlt';
import { useAppDispatch, useAppSelector } from '../../../Redux/Store';
import { useParams, useSearchParams } from 'react-router-dom';
import { fetchAllProducts } from '../../../Redux/Customer/ProductSlice';
import CustomTheme from '../../../Theme/CustomTheme';
import Navbar from '../../Component/Navbar';

function Products() {

  const theme = useTheme();

  const [showSmallScreenFilter, setShowSmallScreenFilter] = useState(false);
  const handleSmallScreenFilter = ()=>{
    setShowSmallScreenFilter(!showSmallScreenFilter);
  }



    const smallScreeFilterRef = useRef<HTMLDivElement | null>(null); // Define the type explicitly


    // close small Scree Filter  when outside click
    useEffect(() => {
      const handleClickOutside = (event:any) => {
        if (smallScreeFilterRef.current && !smallScreeFilterRef.current.contains(event.target)) {
          setShowSmallScreenFilter(false);
        }
      };
  
      document.addEventListener("mousedown", handleClickOutside);
      return () => {
        document.removeEventListener("mousedown", handleClickOutside);
      };
    }, [smallScreeFilterRef]);



    const dispatch = useAppDispatch();
    const {customerProduct} = useAppSelector(store=>store);

    const [sort, setSort] = useState("");
    const handleSortChange = (e:any)=>{
      setSort(e.target.value);
    }
  

    const {category} = useParams();
    console.log(category);
    const [searchParams, setSearchParams] = useSearchParams();


    useEffect(()=>{
       const [minPrice,maxPrice] = searchParams.get("price")?.split("-") || []
       const colors = searchParams.get("color");
       const minDiscount = searchParams.get("discount")? Number(searchParams.get("discount")) :undefined;
       const page = searchParams.get("page") || 1;
       const stocks = searchParams.get("stock");
       
       const params = {
        categori: category,
        colors: colors || "",
        minPrice: minPrice ? Number(minPrice) : undefined,
        maxPrice: maxPrice ? Number(maxPrice) : undefined,
        minDiscount:minDiscount,
        stock:stocks,
        pageNumber : Number(page)-1,
        sort:sort
       }
       
       dispatch(fetchAllProducts(params));
    },[category, searchParams, sort])

   
    
    const updateFilterParams = (name: string, value: any) => {
      const newSearchParams = new URLSearchParams(searchParams);
    
      if (value) {
        newSearchParams.set(name, value.toString());
      } else {
        newSearchParams.delete(name);
      }
    
      setSearchParams(newSearchParams);
    };

  return (
    <div>
     <ThemeProvider theme={CustomTheme}>
      <Navbar/>
     </ThemeProvider>

     <div className='-z-10 mt-10'>
        <div>
            <h1 className='text-3xl text-center font-bold uppercase space-x-2 pb-5 [text-shadow:2px_2px_2px_var(--tw-shadow-color)] shadow-pink-600'>
              { customerProduct.pageable?.content?.length>0 ? customerProduct.pageable?.content?.[0].category?.name :
               "Our Products"}
            </h1>

        </div>

        <div className='lg:flex'>
           <section className='filter-section hidden lg:block w-[20%]'>
             <FilterSection/>
           </section>

          <div className='w-full lg:w-[80%] space-y-5'>
            <div className='flex items-center justify-between px-9 h-[40px]'>
              <div className='relative w-full'>
                 <div className='lg:hidden '>
                    <IconButton onClick={handleSmallScreenFilter}> <FilterAltIcon/> </IconButton>
                    {showSmallScreenFilter && 
                      <Box ref={smallScreeFilterRef}  className="shadow-2xl relative z-50 top-[26rem] ">
                        <FilterSection/>
                      </Box>
                    }
                 </div>                 
              </div>

              <FormControl  sx={{width:"200px"}}>
                <InputLabel id="demo-simple-select-label">Sort</InputLabel>
                <Select
                  labelId="demo-simple-select-label"
                  id="demo-simple-select"
                  value={sort}
                  label="Age"
                  onChange={(e)=>handleSortChange(e)}
                >
                  <MenuItem value={"price_low"}>Price : Low - High</MenuItem>
                  <MenuItem value={"price_high"}>Price : High - Low</MenuItem>
                </Select>
              </FormControl>

            </div>

            <Divider/>

           { customerProduct.pageable?.content?.length>0 ?
             <section className='products-section grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 gap-3 lg:gap-6 justify-center '>
             { customerProduct.pageable?.content?.length>0 &&  customerProduct.pageable?.content?.map((item:any)=><ProductCard key={item.id} productItem={item}/>) }
           </section> : 
            <div className='h-[80vh] md:h-[50%] px-5 flex flex-col gap-5 justify-center items-center bg-gray-100'>
               <img src="data:image/png;base64," alt="No Products" />

               <div className='space-y-2 flex flex-col items-center'>
                <p className='font-semibold text-xl text-green-500'>No records has been added yet.</p>
               
                 <p className='opacity-60 font-semibold'>We are Coming soon. Explore other products on our site.  </p>
               </div>
            </div>
           }

 
            {customerProduct.pageable?.content?.length > 0 && (
              <div className="flex justify-center py-8">
                <Pagination
                  variant="outlined"
                  color="primary"
                  count={customerProduct.pageable?.totalPages}
                  onChange={(event, page) => updateFilterParams("page", page)}
                />
              </div>
            )}

          </div>

        </div>
      </div>

    </div>
   
  )
}

export default Products;
