...
function Cart() {
  ...
  return (
    <>
      ...
      <div className='py-10 px-5 md:px-28 min-h-screen'>
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-5 '> 
            <div className="cartItemsection lg:col-span-2 space-y-3">
              {cart?.userCart?.cartItems && cart.userCart.cartItems.length > 0 ? (
                <div>
                  {cart.userCart.cartItems.map((item, i) => (
                    <CartItem key={item.id} item={item} />
                  ))}
                </div>
              ) : (
                <div className='h-screen px-5 flex flex-col gap-5 justify-center items-center bg-gray-100'>
                      ...
                      <p className='font-semibold text-xl text-green-500'>No records has been added yet.</p>
                        ...
                        <p className='opacity-60 font-semibold'>Add product to cart. Explore other products on our site.  </p>
                      </div>
                </div>
              )}
            </div>      
            ...
            <div className='col-span-1 lg:h-screen lg:sticky top-0 text-sm space-y-3'>
              <div className='border rounded-md px-5 py-3 space-y-5'>
                  <div className='space-y-3'>
                      <div className=' flex gap-3 text-sm items-center'>
                          <LocalOffer sx={{color:teal[600], fontSize:"17px"}} />
                          <span className='text-slate-600'>Apply Coupon</span>
                      </div>
                      ...
                      { true? <div className='flex justify-between items-center'>
                        ...
                        <div className='p-1 pl-5 pr-3 border rounded-md flex gap-1 items-center'>
                            <span className=''>Dart28O Applied</span>
                            <IconButton color='error' size='small'><Close className='text-red-500' /></IconButton>
                        </div>
                      }
                  </div>
              </div>
